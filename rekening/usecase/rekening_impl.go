...
import (
	...
	"m-banking/internal"
	repositories "m-banking/nasabah/repository"
...
func NewAccountNumberUsecaseImpl(accountNumberRepository repository.AccountNumberRepository) usecase.AccountNumberUsecase {
	return &accountNumberUsecase{accountNumberRepository, repositories.NewCustomerRepositoryImpl(postgresql.DB), internal.NewPublisherImpl(rabbitmq.RabbitMQChannel), sync.WaitGroup{}}
}

func (s *accountNumberUsecase) GetBalanceUsecase(ctx context.Context, accountNumber int) (*accNumberdto.AccountNumberResponse, error) {
...
func (s *accountNumberUsecase) DepositUsecase(ctx context.Context, account accNumberdto.AccountNumberRequest) (*accNumberdto.AccountNumberResponse, error) {
	...
	go func() {
		defer s.wg.Done()
		publishError = s.publisher.PublishMessage(ctx, deposit.ID, "D", account.Amount, "deposit")
	}()
...
func (s *accountNumberUsecase) CashoutUsecase(ctx context.Context, account accNumberdto.AccountNumberRequest) (*accNumberdto.AccountNumberResponse, error) {
	...
	go func() {
		defer s.wg.Done()
		publishError = s.publisher.PublishMessage(ctx, cashout.ID, "C", account.Amount, "cashout")
	}()
...
func (s *accountNumberUsecase) TransferUsecase(ctx context.Context, transfer accNumberdto.TransferBalanceRequest) (*accNumberdto.TransferBalanceResponse, error) {
	...
	for _, accountNumberID := range accountNumbersID {
		...
		go func(accountNumberID int) {
			defer s.wg.Done()
			publishError = s.publisher.PublishMessage(ctx, accountNumberID, "T", transfer.Amount, "transfer")
		}(accountNumberID)
	}
